...
/* ── Postea a una plataforma ya con credenciales resueltas. Devuelve { postId, message, isDraft? }
   o lanza un Error. La usan tanto el botón manual "Publicar" (publishContent) como el cron de
   auto-publicación (autoPublishScheduler.js) — misma lógica, un solo lugar. ── */
async function postToPlatform(platform, credentials, { content, imageUrl, videoUrl }) {
  /* ── Facebook ── */
  if (platform === "facebook") {
    ...
    return { postId: fbData.id, message: "¡Publicado en Facebook exitosamente!" };
  }

  /* ── Instagram ── */
  if (platform === "instagram") {
    if (!imageUrl) {
      throw new Error("Instagram requiere una imagen. Genera una imagen para esta campaña primero.");
    }
    const { pageId, accessToken } = credentials;
    ...
    return { postId: publishData.id, message: "¡Publicado en Instagram exitosamente!" };
  }
  ...
  if (platform === "twitter") {
    ...
    return { postId: twitterData.data?.id, message: "¡Tweet publicado exitosamente!" };
  }
  ...
  if (platform === "linkedin") {
    ...
    return { postId: linkedinData.id, message: "¡Publicado en LinkedIn exitosamente!" };
  }

  /* ── TikTok ── */
  if (platform === "tiktok") {
    if (!videoUrl) {
      throw new Error("TikTok requiere un video. Genera un video para esta campaña primero.");
    }
    const { accessToken } = credentials;
...
const publishContent = async (req, res) => {
  try {
    ...
    const result = await postToPlatform(platform, credentials, { content, imageUrl, videoUrl });
...
module.exports = { saveSocialCredentials, getSocialCredentials, disconnectSocialCredentials, publishContent, postToPlatform };
